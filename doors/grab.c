		...
		if (repfd > -1) {
			...
			if (!strncasecmp(qwkblock, bbsid, strlen(bbsid))) {
				while (read(repfd, &qwkblock, 128)) {
					...
					dd_setmprs(d, &mp);
					sprintf(qbuf, "%s/messages/base%3.3d/msgbase.dat", conf->CONF_PATH, base->MSGBASE_NUMBER);
					...
					sprintf(qbuf, "%s/messages/base%3.3d/msg%5.5d", conf->CONF_PATH, base->MSGBASE_NUMBER, msg.MSG_NUMBER);

					msgfd = open(qbuf, O_CREAT | O_WRONLY, 0644);
					if (toupper(base->MSGBASE_FN_FLAGS) == 'E') {
						char ub[128];
						char ebuf[1024];
						int uq;
...
static int makeqwk(void)
{
	...
	fprintf(ctrl, "%s\r\n%s\r\n%s\r\n%s, Sysop\r\n", maincfg.CFG_BOARDNAME, "Location", "Phone", maincfg.CFG_SYSOPNAME);
	fprintf(ctrl, "00000,%s\r\n%-2.2d-%2.2d-%-4.4d,%-2.2d:%-2.2d:%-2.2d\r\n%s\r\n\r\n0\r\n", bbsid, tm->tm_mon + 1, tm->tm_mday, 1900 + tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, username);
			...
			for (bcnt = conf->CONF_MSGBASES; bcnt; bcnt--, base++) {
				...
				if (dd_isbasetagged(d, conf->CONF_NUMBER, base->MSGBASE_NUMBER)) {
					...
					if (mp.msp_high > lp.lrp_read) {
						...
						sprintf(mailbuf, "%s/messages/base%3.3d/msgbase.dat", conf->CONF_PATH, base->MSGBASE_NUMBER);
						basefd = open(mailbuf, O_RDONLY);
						if (basefd == -1)
							continue;
...
static int procqwkm(struct DayDream_Message *daheader)
{
	...
	sprintf(msgname, "%s/messages/base%3.3d/msg%5.5d", conf->CONF_PATH, base->MSGBASE_NUMBER, daheader->MSG_NUMBER);
	if (stat(msgname, &st) == -1)
		return 0;

	msgf = fopen(msgname, "r");
	if (!msgf)
		return 0;
	...
	fprintf(msgdfd, "%c%-7d%2.2d-%2.2d-%2.2d%-2.2d:%-2.2d%-25.25s%-25.25s%-25.25s%-12.12s%-8.8s", msgstatus, daheader->MSG_NUMBER, tm->tm_mon + 1, tm->tm_mday, tm->tm_year % 100, tm->tm_hour, tm->tm_min, rec, daheader->MSG_AUTHOR, daheader->MSG_SUBJECT, " ", " ");
