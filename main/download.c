...
int sendfiles(char *list, char *lastf, size_t flen)
{
	...
	if (protocol->PROTOCOL_TYPE == 1) {
		...
		snprintf(bigbuf, sizeof bigbuf, "%s/utils/hydracom dev %s ski %s/data/paths.dat rec %s/ res %s send @%s", origdir, ttyname(serhandle), conference()->conf.CONF_PATH, currnode->MULTI_TEMPORARY, parbuf, list);
		runstdio(bigbuf, -1, 4);
		killflood();
	} else if (protocol->PROTOCOL_TYPE == 3) {
		snprintf(bigbuf, sizeof bigbuf, "%s/utils/smodem -p %s -t -X -d %s/ @%s/data/paths.dat -z %s -e 0 -u @%s", origdir, ttyname(serhandle), currnode->MULTI_TEMPORARY, conference()->conf.CONF_PATH, parbuf, list);
		runstdio(bigbuf, -1, 4);
		killflood();
