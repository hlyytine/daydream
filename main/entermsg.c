...
int entermsg(struct DayDream_Message *msg, int reply, char *params)
{
	...
	*header.MSG_ATTACH = 0;
	...
	snprintf(ebuf, sizeof ebuf, "%s/messages/base%3.3d/msgbase.dat", conference()->conf.CONF_PATH, current_msgbase->MSGBASE_NUMBER);
	...
	if ((msgfd = open(ebuf, O_RDWR | O_CREAT, 0666)) == -1) {
	...
	snprintf(ebuf, sizeof ebuf, "%s/messages/base%3.3d/msg%5.5d", conference()->conf.CONF_PATH, current_msgbase->MSGBASE_NUMBER, header.MSG_NUMBER);

	if ((msgfd = open(ebuf, O_RDWR | O_CREAT | O_TRUNC, 0666)) == -1) {
		DDPut(sd[emwriteerrstr]);
		free(lineedmem);
		return 0;
	}
...
void getmsgptrs(void)
{
	...
	char gmpbuf[1024];
	snprintf(gmpbuf, sizeof gmpbuf, "%s/messages/base%3.3d/msgbase.ptr", conference()->conf.CONF_PATH, current_msgbase->MSGBASE_NUMBER);
...
int setmsgptrs(void)
{
	...
	if ((msgfd = open(gmpbuf, O_RDWR | O_CREAT, 0666)) == -1) {
