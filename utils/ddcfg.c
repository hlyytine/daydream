...
int main(int argc, char *argv[])
{
	...
	if (cfgflag2("Y","ALLOW2FTPUSER")) maincfg.CFG_FLAGS |= (1L << 12);
...
nextconf:
	...
	if (cfgflag2("Y","CONF_DEFFSCAN")) {
		...
		selcfg[2048+((conf.CONF_NUMBER-1)/8)] |= (1L<<(conf.CONF_NUMBER-1)%8);
	}
...
nextsec:
	...
	if (cfgflag2("Y","SECF_HYDRA")) sec.SEC_ACCESSBITS1 |= (1L<<18);
